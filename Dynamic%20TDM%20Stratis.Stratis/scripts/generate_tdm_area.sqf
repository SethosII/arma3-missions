 ...
 */
 ...
 if (isServer) then {
	...
	private ["_maxSizeX", "_maxSizeY", "_objectName", "_objectWidth", "_objectLength", "_objectHeight", "_inWater", "_leftTopX", "_leftTopY", "_leftBottomX", "_leftBottomY", "_rightTopX", "_rightTopY", "_rightBottomX", "_rightBottomY", "_bottomLength", "_rotationBottom", "_boundary", "_topLength", "_rotationTop", "_leftLength", "_rotationLeft", "_rightLength", "_rotationRight", "_respawnMarker"];
	...
	for "_x" from 0 to _bottomLength + _objectWidth step _objectWidth do {
		...
		_boundary = createVehicle [_objectName, [_leftBottomX + sin _angle * (_objectLength / 2) + cos _angle * _x, _leftBottomY - cos _angle * (_objectLength / 2) + sin _angle * _x, _objectHeight], [], 0, "CAN_COLLIDE"];
		_boundary setDir (360 - _rotationBottom);
	};
	...
	for "_x" from 0 to _topLength + _objectWidth step _objectWidth do {
		...
		_boundary = createVehicle [_objectName, [_rightTopX - sin _angle * (_objectLength / 2) - cos _angle * _x, _rightTopY + cos _angle * (_objectLength / 2) - sin _angle * _x, _objectHeight], [], 0, "CAN_COLLIDE"];
		_boundary setDir (360 - _rotationTop);
	};
	...
	for "_x" from 0 to _leftLength + _objectWidth step _objectWidth do {
		...
		_boundary = createVehicle [_objectName, [_leftTopX - cos _angle * (_objectLength / 2) + sin _angle * _x, _leftTopY - sin _angle * (_objectLength / 2) - cos _angle * _x, _objectHeight], [], 0, "CAN_COLLIDE"];
		_boundary setDir (360 - _rotationLeft);
	};
	...
	for "_x" from 0 to _rightLength + _objectWidth step _objectWidth do {
		...
		_boundary = createVehicle [_objectName, [_rightBottomX + cos _angle * (_objectLength / 2) - sin _angle * _x, _rightBottomY + sin _angle * (_objectLength / 2) + cos _angle * _x, _objectHeight], [], 0, "CAN_COLLIDE"];
		_boundary setDir (360 - _rotationRight);
	};
